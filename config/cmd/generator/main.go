...
//func (pi *protobufImportModuleProvider) ResolveImport()

func (pi *protobufImportModuleProvider) Provide(module string) (io.Reader, error) {
	moduleNsPieces := strings.Split(module, "/")
	moduleNs := strings.Join(moduleNsPieces[:len(moduleNsPieces)-1], "/")

	modulePath := pi.dir + filepathSeparator + module
	if moduleNs == "google/protobuf" {
	...
	for _, m := range pf.Messages {
		...
		for _, mf := range m.Fields {
			...
			if strings.HasPrefix(mf.Type.Name(), "google.protobuf.") {
				...
				//c += fmt.Sprintf("    if x.%s == nil { x.%s = new(wrappers.%s) }\n", fieldName, fieldName, _type)
				c += fmt.Sprintf(
