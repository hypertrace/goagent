...
func TestSetScalarAttributeSuccess(t *testing.T) {
	...
	assert.Equal(t, "value_1", attrs.Get("rpc.request.metadata.key_1").AsString())
}
...
func TestSetMultivalueAttributeSuccess(t *testing.T) {
	...
	readbackSpan := flusher()[0]
	attrs := internal.LookupAttributes(readbackSpan.Attributes)
	assert.Equal(t, "value_1", attrs.Get("rpc.request.metadata.key_1[0]").AsString())
	assert.Equal(t, "value_2", attrs.Get("rpc.request.metadata.key_1[1]").AsString())
}
