...
type Filter interface {
	...
	EvaluateURLAndHeaders(span sdk.Span, url string, headers map[string][]string) filterutils.FilterResult

	// EvaluateBody can be used to evaluate the body content
	EvaluateBody(span sdk.Span, body []byte, headers map[string][]string) filterutils.FilterResult

	// Evaluate can be used to evaluate URL, headers and body content in one call
	Evaluate(span sdk.Span, url string, body []byte, headers map[string][]string) filterutils.FilterResult
}
