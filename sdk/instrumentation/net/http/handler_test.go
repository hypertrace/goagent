...
func TestServerRequestIsSuccessfullyTraced(t *testing.T) {
	...
	wh, _ := WrapHandler(h, mock.SpanFromContext, &Options{}, map[string]string{"foo": "bar"}, &metricsHandler{}).(*handler)
	wh.dataCaptureConfig = emptyTestConfig
	ih := &mockHandler{baseHandler: wh}
...
func TestServerRequestFilter(t *testing.T) {
				...
				Filter: mock.Filter{
					Evaluator: func(span sdk.Span) result.FilterResult {
						...
						assert.Equal(t, base64.RawStdEncoding.EncodeToString([]byte("haha")),
							span.GetAttributes().GetValue("http.request.body.base64"))
			...
			options: &Options{
				Filter: mock.Filter{
					Evaluator: func(span sdk.Span) result.FilterResult {
						...
						assert.Nil(t, span.GetAttributes().GetValue("http.request.body.base64"))
						return result.FilterResult{}
					},
				},
			},
	...
	for name, tCase := range tCases {
		t.Run(name, func(t *testing.T) {
			...
			wh, _ := WrapHandler(h, mock.SpanFromContext, tCase.options, map[string]string{}, &metricsHandler{}).(*handler)
			ih := &mockHandler{baseHandler: wh}
...
func TestProcessingBodyIsTrimmed(t *testing.T) {
	...
	wh.dataCaptureConfig.BodyMaxSizeBytes = config.Int32(int32(bodyMaxProcessingSizeBytes))
